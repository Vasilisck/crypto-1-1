val c1 = ""
val c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
val c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
val c4 = ""
val c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
val c6 = ""
val c7 = ""
val c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
val c9 = ""
val c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
val cTarget = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"

def possibleKeys(symbols: List[Int]): List[Int] = {
  (0 to 255).map(c => {
    //shitlevel is special param, that defines how bad is result
    val shitLevel = symbols.map(symbol => {
      val mi = c ^ symbol
      if ((mi > 64 && mi < 91) || (mi > 96 && mi < 123)) 0 else
      if (mi == 32 || mi == 44 || mi == 46) 1 else
      if (mi > 126 || mi < 32) 9000 else
      if (mi > 47 && mi < 58 || mi == 63) 2 else
        3
    })
    (shitLevel.sum, c)
  }).filter(_._1 < 15).toList.sortBy(_._1).map(_._2)
}

val cipherList = List(c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, cTarget)

val decCipherList = cipherList
  .map(cipher => cipher.grouped(2)
    .map(Integer.parseInt(_, 16)))
  .map(_.toArray)
val keys = (0 to 82).map(target => {
  val targetSymbol = decCipherList.map(a => a(target))
  val posRes = possibleKeys(targetSymbol)
  if(target > 30) {
    println(target + " have size " + posRes.length + " possible results: " + posRes)
  }
  // posRes.foreach(res => println(target + ": " + res))
  posRes.headOption.getOrElse(-1)
})
println("found " + keys.filterNot(_ == 0).size + " keys from 120 symbols")

val trueKeys = keys.zipWithIndex.map(kv => {
    if(kv._2 == 1) 57 else
    if(kv._2 == 5) 219 else
    if(kv._2 == 7) 204 else
    if(kv._2 == 7) 204 else
    if(kv._2 == 8) 152 else
    if(kv._2 == 10) 53 else
    if(kv._2 == 11) 42 else
    if(kv._2 == 14) 149 else
    if(kv._2 == 18) 206 else
    if(kv._2 == 19) 120 else
    if(kv._2 == 21) 127 else
    if(kv._2 == 25) 127 else
    if(kv._2 == 33) 176 else
    if(kv._2 == 34) 51 else
    if(kv._2 == 35) 154 else
    if(kv._2 == 41) 156 else
  //  if(kv._2 == 37) 180 else
  kv._1
})
//val q = decCipherList(2)

val mi = (0 to 10)
  .map(n => (trueKeys zip decCipherList(n))
    .map(kc => {
      if (kc._1 == -1) 32 else kc._1 ^ kc._2
    })
    .map(_.toChar)).toList

println()
mi.foreach(println)

println(" 11111!~!!! " + decCipherList(0)(41))
println(" 11111!~!!! " + mi(0)(41))

println(mi.last.mkString)
//println(mi(3))
//val res = mi.map(_.mkString)

//res.foreach(println)
//println((90^50).toChar)